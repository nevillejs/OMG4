...
def test_export():
    ...
    save_dict = {
        ...
        "scale_htable": [None], # Mock huffman table not needed for this test if we mock huffman_decode
    ...
    export_onnx.huffman_decode = MagicMock(side_effect=lambda code, table: code.astype(np.uint16)) # Just pass through for test
    
    print("Testing ONNX export...")
    try:
        wrapper = export_onnx.MLPInference(
        ...
        torch.onnx.export(
            wrapper,
            (torch.randn(100, 3), torch.randn(100, 3), torch.randn(100, 3), torch.tensor([0.5])),
            onnx_path,
